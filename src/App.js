        ...
        <Container fluid="true" style={{margin:"10px", position: "relative", top: (nav.current || {offsetHeight:0}).offsetHeight}}>
            ...
            <Card style={{width:"60%"}}>
                ...
                <Form.Check type="checkbox" defaultChecked={true} label="Auto-send (automatically send updates when values are changed)" onChange={(event)=>{setAutoUpdate(event.target.checked)}} />
                    <SliderInput overrideVal={speedOverride.current} override={enableController.current} val={vals.speed} updateVal={updateVal} var={"speed"} name={"Speed setpoint (rad/s)"} dispScaler={1} scaler={2} defaultValue={50} offset={50} deadzone={2} zero={true} />
                    ...
                    <SliderInput updateVal={updateVal} var={"stop_distance"} name={"Stopping distance (m)"} scaler={100} defaultValue={0} dispScaler={1} offset={0} />
                    <SliderInput updateVal={updateVal} var={"stop_accel"} name={"Stopping accel (m/s^2)"} scaler={5} defaultValue={75} dispScaler={1} offset={0} />
